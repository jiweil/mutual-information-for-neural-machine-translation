function[total_cost,grad]=softmax(h,batch,parameter,isTraining)
    ...
    step_size=1;
    for Begin=1:step_size:MaxLenTarget
        End=Begin+step_size-1;
        if End>MaxLen
            End=MaxLen;
        end
        N_examples=size(Word,1)*(End-Begin+1);
        predict_Words=reshape(Word(:,MaxLenSource+Begin:MaxLenSource+End),1,N_examples);
        mask=reshape(Mask(:,MaxLenSource+Begin:MaxLenSource+End),1,N_examples);%remove positions with no words
        h_t=[h{:,Begin:End}];
        [cost,grad_softmax_h]=batchSoftmax(h_t,mask,predict_Words,parameter);
        total_cost=total_cost+cost;
        grad.soft_W=grad.soft_W+grad_softmax_h.soft_W;
        for j=Begin:End
            grad.ht{1,j}=grad_softmax_h.h(:,(j-Begin)*size(Word,1)+1:(j-Begin+1)*size(Word,1)); 
        end
    end
    if parameter.CheckGrad==1 total_cost=total_cost/size(Word,1);
    end
