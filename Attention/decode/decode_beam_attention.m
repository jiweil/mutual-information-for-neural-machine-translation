function[]=decode_beam_attention(parameter,TestBatches,filename)
    disp('decode')
    ...
    %for batch_index=1:1000
        ...
        [lstms,last_h_t,last_c_t,source_lstm]=Forward(batch,parameter,0,1);
        source_lstm=permute(source_lstm,[1,3,2]);
        atten=WorkAtten(source_lstm,last_h_t{parameter.layer_num},batch);
        h_=parameter.nonlinear_f(parameter.Atten_W*[last_h_t{parameter.layer_num};atten.vector]);
        zeroState=zeroMatrix([parameter.hidden,parameter.beamSize]);
        
        [first_scores,first_words]=BeamStep(parameter,h_,1,[],1);
%         first_words
%         first_scores
        ...
        for position=1:max_length
            ...
            for ll=1:parameter.layer_num
                if ll == 1
                    ...
                    if size(h_,2)~=parameter.beamSize
                        x_t=[x_t;repmat(h_,1,parameter.beamSize)];
                    else x_t=[x_t;h_];
                    end
                else
            ...
            atten=WorkAtten(source_lstm,beamStates{parameter.layer_num}.h_t,batch);
            h_=parameter.nonlinear_f(parameter.Atten_W*[beamStates{parameter.layer_num}.h_t;atten.vector]);
            
            [all_next_scores,all_next_words]=BeamStep(parameter,h_,0,words,position+1);
